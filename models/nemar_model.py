...
class NEMARModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if True: # modified this so that losses print on testing data
    ...
    def define_networks(self):
        ...
        if True:  # modified this from 'if self.isTrain:' since I wanted to see discriminator losses for test images
    ...
    def reset_weights(self):
        # resets discriminator/translation network's weights during training.
    ...
    def backward_T_and_R(self):
        ...
        # modified to multiply by stn_reg_term.mean() for multi-GPU training 
        self.loss_smoothness = self.opt.lambda_smooth * self.stn_reg_term.mean()

        loss = self.loss_L1_TR + self.loss_L1_RT + self.loss_GAN_TR + self.loss_GAN_RT + self.loss_smoothness

        #modified to use loss.mean() for multi-GPU training 
        loss = loss.mean()
        loss.backward()
